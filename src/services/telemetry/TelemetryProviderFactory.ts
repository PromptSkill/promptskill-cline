...
export class TelemetryProviderFactory {
	...
	public static getDefaultConfigs(): TelemetryProviderConfig[] {
		if (isPromptSkillWorkspace() && shouldUsePromptSkillNoOpTelemetryProviders()) {
			// Candidate behavior is evaluated by PromptSkill's own assessment telemetry.
			// Do not initialize Cline SaaS analytics providers inside candidate workspaces.
			return [{ type: "no-op" }]
		}
