...
import { isPromptSkillUniversalGenericWorkspace } from "@/integrations/promptskill/workspace"
import {
...
export class StateManager {
	...
	public static async initialize(context: ExtensionContext): Promise<StateManager> {
		...
		try {
			...
			if (isPromptSkillUniversalGenericWorkspace()) {
				// Universal generic workspaces warm Theia state only. Candidate/session API
				// settings are injected and persisted when hydration claims the workspace.
			...
			// ---------------------------------------------------------------------
			// PromptSkill: bake in authoritative candidate API configuration from workspace env.
			// ---------------------------------------------------------------------
			...
			try {
				const promptSkillWorkspaceEnvironment = promptSkillCandidateWorkspaceEnvironmentFromProcessEnv()

				StateManager.instance.setApiConfiguration(
					promptSkillApiConfigurationFromWorkspaceEnvironment(
						StateManager.instance.constructApiConfigurationFromCache(),
						promptSkillWorkspaceEnvironment,
					),
				)

				// PromptSkill candidate workspaces require native tool calls on every turn,
				// so seed the persisted Cline toggle from the authoritative workspace env.
				StateManager.instance.setGlobalState(
					"nativeToolCallEnabled",
					promptSkillNativeToolCallsEnabled(promptSkillWorkspaceEnvironment),
				)
			} catch (e) {
