		...
		try {
			...
			if (isContextWindowExceededError && !this.taskState.didAutomaticallyRetryFailedApiRequest) {
				...
				if (
					...
					if (!isClineProviderInsufficientCredits && !isAuthError && !isAssessmentBudgetError) {
						await this.say(
							"error_retry",
							JSON.stringify({
								attempt: 3,
			...
			try {
				for await (const chunk of stream) {
					if (!chunk) {
						...
						throw new Error("Provider stream yielded an undefined chunk.")
					}

					if (typeof chunk !== "object" || !("type" in chunk)) {
						console.error("[Cline] stream yielded malformed chunk:", chunk)
						throw new Error(`Provider stream yielded a malformed chunk: ${JSON.stringify(chunk)}`)
					}
