...
export async function initializeWebview(controller: Controller, _request: EmptyRequest): Promise<Empty> {
	try {
		if (isPromptSkillWorkspace()) {
			// PromptSkill injects one authoritative OpenAI-native model via env and does not expose
			// Cline's provider picker or MCP marketplace, so these catalog refreshes only add cold-start work.
			return Empty.create({})
		}
