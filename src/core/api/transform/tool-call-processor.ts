...
interface OpenAIToolParamOptions {
	requireToolUse?: boolean
}

export function getOpenAIToolParams(tools?: OpenAITool[], options?: OpenAIToolParamOptions) {
	return tools?.length
		? {
				tools,
				//
				tool_choice: tools
					? ((options?.requireToolUse ? "required" : "auto") as ChatCompletionToolChoiceOption)
					: undefined,
