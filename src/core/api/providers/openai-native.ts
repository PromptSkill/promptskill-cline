...
export class OpenAiNativeHandler implements ApiHandler {
		...
		switch (model.id) {
			...
			case "gpt-5.1": {
				const stream = await client.chat.completions.create({
					...
					stream_options: { include_usage: true },
					reasoning_effort: (this.options.reasoningEffort as ChatCompletionReasoningEffort) || "medium",
					...getOpenAIToolParams(tools, { requireToolUse: true }),
				})
	...
	private async *createResponseStream(
		...
		const stream = await client.responses.create({
			...
			stream: true,
			tools: responseTools,
			tool_choice: responseTools?.length ? "required" : undefined,
	...
	getModel(): { id: string; info: ModelInfo } {
		...
		const promptSkillConfiguredModelId = process.env.PROMPTSKILL_CLINE_OPENAI_MODEL_ID
		...
		// PromptSkill pins the workspace model via env, and that model may move ahead of
		// Cline's hardcoded native OpenAI map. Preserve the configured model id instead of
		// silently drifting to Cline's default native model when the upstream map lags behind.
		if (modelId && promptSkillConfiguredModelId && modelId === promptSkillConfiguredModelId) {
			return {
				id: modelId,
