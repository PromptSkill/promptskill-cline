...
export class OpenAiHandler implements ApiHandler {
			...
			try {
				...
				if (
					...
					this.client = new AzureOpenAI({
						...
						fetch: this.fetchWithErrorBodyPreservation, // Use configured fetch with proxy support
					})
				} else {
	...
	async *createMessage(systemPrompt: string, messages: ClineStorageMessage[], tools?: ChatCompletionTool[]): ApiStream {
		...
		for await (const chunk of stream) {
			if (!chunk || typeof chunk !== "object") {
				console.error("[OpenAiHandler] OpenAI-compatible stream yielded malformed SDK chunk:", chunk)
				throw new Error("OpenAI-compatible stream yielded a malformed SDK chunk.")
			}
	...
	private readonly fetchWithErrorBodyPreservation: typeof globalThis.fetch = async (input, init) => {
		...
		error.code = typeof parsedErrorBody?.code === "string" ? parsedErrorBody.code : undefined
		error.request_id = typeof parsedErrorBody?.request_id === "string" ? parsedErrorBody.request_id : headers["x-request-id"]
		error.details = parsedErrorBody ?? errorBodyText
		error.response = {
