...
export async function initialize(context: vscode.ExtensionContext): Promise<WebviewProvider> {
	...
	const promptSkillWorkspace = isPromptSkillWorkspace()

	await ErrorService.initialize()

	if (!promptSkillWorkspace) {
		// PromptSkill workspaces use env-pinned candidate config or golden prebuild state.
		// Cline SaaS services are unnecessary on the cold path and can delay AI panel startup.
