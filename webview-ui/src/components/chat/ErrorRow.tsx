...
const DEFAULT_CANDIDATE_AI_BUDGET_MESSAGE =
	"This message could not be sent because it may exceed the AI budget for this assessment. Please retry with a shorter message, or try to continue the assessment without AI."
const GENERIC_BUDGET_ERROR_MESSAGES = new Set(["connection error.", "409 status code (no body)"])
		...
		switch (errorType) {
			...
			case "mistake_limit_reached":
				...
				if (rawApiError) {
					...
					const isAssessmentBudgetError = clineError?.isErrorType(ClineErrorType.AssessmentBudget)
					const detailsMessage =
						typeof clineError?._error?.details?.message === "string" ? clineError._error.details.message.trim() : undefined
					const primaryErrorMessage =
						typeof clineError?._error?.message === "string"
							? clineError._error.message.trim()
							: typeof clineError?.message === "string"
								? clineError.message.trim()
								: undefined
					const errorMessage =
						isAssessmentBudgetError &&
						((detailsMessage && GENERIC_BUDGET_ERROR_MESSAGES.has(detailsMessage.toLowerCase())) ||
							(primaryErrorMessage && GENERIC_BUDGET_ERROR_MESSAGES.has(primaryErrorMessage.toLowerCase())))
							? DEFAULT_CANDIDATE_AI_BUDGET_MESSAGE
							: detailsMessage || primaryErrorMessage || rawApiError
